 $(document).ready(function () {
 	$(document).on('click', '.edit-report-file', function(a){

 		a.preventDefault();

		let id = $(this).data('id');   // it will get id of clicked row

		$("#modaltitle").html("<i class='fas fa-edit fa-fw'></i> Change Report's Attachment");

		$('#crud-content').html(''); // leave it blank before ajax call
		$('#modal-loader').show();      // load ajax loader

		$.ajax({
			url: '/user/research/card/report/file/'+id+'/edit',
			type: 'GET',
			dataType: 'html'
		})
		.done(function(data){
