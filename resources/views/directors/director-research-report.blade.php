...
@section('content')
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h3 class="h3 mb-0 text-gray-800">Research</h3>
        <div class="btn-group" role="group" aria-label="Basic example">
			<a href="{{ route('director-research') }}" class="btn btn-sm btn-secondary"><i class="fas fa-arrow-left"></i> Back</a>
            <a href="{{ route('director-research-reports.create', $card->id) }}" class="btn btn-sm btn-primary submit-report {{ ($card->is_lock)? 'disabled' : '' }}"><i class="fas fa-upload fa-sm text-white-50"></i> Submit report</a>
        </div>
	</div>
	<hr>
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
	...
	<div class="card shadow mb-4">
		...
		<div class="card-body">
            <a href="{{ route('director-submitted-research-report.export-pdf',encrypt($card->id)) }}" class="btn btn-info btn-sm" target="_blank">
		...
		<script type="text/javascript">
			$(document).ready(function() {
			    ...
			    $('#researchCardDataTable').DataTable({
			    	...
			    	"serverSide": false,
			    	"ajax":{
			    		"url": "{{ route('director-research-reports.data',[$card->id,'research']) }}",
			    		"type": "GET"
			    	},
			    	"columns": [
			    	{ "data": "id" },
