...
@section('content')
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="btn-group" role="group" aria-label="Basic example">
			<a href="{{ route('user-extension') }}" class="btn btn-sm btn-secondary"><i class="fas fa-arrow-left"></i> Back</a>
            <a href="{{ route('user-extension-reports.create', $card->id) }}" class="btn btn-sm btn-primary submit-report {{ ($card->is_lock)? 'disabled' : '' }}"><i class="fas fa-upload fa-sm text-white-50"></i> Submit report</a>
        </div>
	</div>
		...
		<script type="text/javascript">
			$(document).ready(function() {
			    ...
			    $('#extensionCardDataTable').DataTable({
			    	...
			    	"serverSide": false,
			    	"ajax":{
			    		"url": "{{ route('user-extension-reports.data',[$card->id,'extension']) }}",
			    		"type": "GET"
			    	},
			    	"columns": [
			    	{ "data": "id" },
